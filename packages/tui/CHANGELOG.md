...
- Fixed a `deleteMiddle`-style offscreen middle-shrink wiping the visible window on Windows hosts with an unknown viewport probe (Windows Terminal, Tabby, Hyper, VS Code, conhost behind ConPTY). The renderer fell through to the deferred-shrink fallback, which pads `newLines` with blank rows at the tail so the viewport keeps its pre-shrink absolute indices — semantics that only preserves the previous visible content for pure trailing shrinks. An offscreen middle deletion shifts every row past the deletion up by `count`, so the padded slice rendered at the old viewport indices showed the post-shift content (and a blank tail pad) instead of the rows the user was looking at. On an at-bottom reader the bottom-anchored tail drifted off-screen (the failing stress oracle saw the entire visible window blanked). The fallback now downgrades to `deferredMutation` when the diff starts above the previous viewport — a literal no-op preserves the visible rows, which are also the correct new bottom-anchored rows because an offscreen deletion only renumbers their absolute indices. The next checkpoint rebuild reconciles scrollback ([#1809](https://github.com/can1357/oh-my-pi/issues/1809)).
